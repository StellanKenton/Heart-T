...
D:\Space\Cosmos\Heart-T\Heart-T 是采集的下位机
D:\Space\Cosmos\Heart-T\HeartThirdCore 是分析用的上位机程序

Device Tool 脚本入口位于 `develop/`：
...
- 在当前 Windows 仓库中，Device Tool 任务应走 `py -3 develop/quick_deploy.py <action>`，不要用 `python3`。
