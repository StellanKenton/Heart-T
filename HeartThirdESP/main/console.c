/************************************************************************************
* @file     : console.c
* @brief    : TCP console command processing.
* @details  : Dispatches complete command lines supplied by the TCP server.
* @author   :
* @date     :
* @version  :
...
/** @brief Reset command assembly and bindings before registering project commands. */
void consoleInit(void) {
    consoleResetInput();
    gConsoleRegisteredCommandCount = 0U;
}

/** @brief Reset partial input on a new TCP client without removing registrations. */
void consoleResetInput(void) {
    gConsoleCommandLength = 0U;
    gConsoleCommandBuffer[0] = '\0';
}
...
/** @brief Feed received TCP bytes to the original command-line dispatcher. */
void consoleFeed(const char *data, uint32_t length) {
    uint32_t lIndex;
    if (data == NULL) {
        return;
    }
    for (lIndex = 0U; lIndex < length; ++lIndex) {
        consolePushChar(data[lIndex]);
    }
}
/**************************End of file********************************/
